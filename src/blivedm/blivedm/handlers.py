...
IGNORED_CMDS = (    # 忽略词条  # 要禁用某些功能在这添加 cmd 里的标签
    'COMBO_SEND',
    'ENTRY_EFFECT',
    'HOT_RANK_CHANGED',
    'HOT_RANK_CHANGED_V2',
...
#BilibiliWebSocket协议参考 https://github.com/lovelyyoshino/Bilibili-Live-API/blob/master/API.WebSocket.md
class BaseHandler(HandlerInterface):
    ...
    def __interact_msg_callback(self, client: client_.BLiveClient, command: dict):  # 需要添加的部分
        return self._on_interact(client, models.InteractMessage.from_command(command['data']))
    ...
    _CMD_CALLBACK_DICT: Dict[
        ...
        # 进入直播间
        'INTERACT_WORD': __interact_msg_callback,   # 需要添加的部分
        # 收到弹幕
    ...
    async def _on_interact(self, client: client_.BLiveClient, message: models.InteractMessage): # 需要添加的部分
        """
        进入直播间
        """
